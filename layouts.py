...
player_comparison_layout = [[psg.Text("pick the two player you want to compare (multiple player comparison coming soon!!!!!!!!!!!!)")]]

def create_player_combobox(players):
    comparison_player_dropbox_combo = psg.Combo(players, font=('Times New Roman', 26), expand_x=True,
                                                enable_events=True,
                                                readonly=False, key='-COMBO-')
    return  [[comparison_player_dropbox_combo, psg.Button('Add'), psg.Button('remove')]]
